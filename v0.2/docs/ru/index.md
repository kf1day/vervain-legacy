...
<a id="seguri">**Сегмент URI**</a> (-пути) – часть пути URI, отделенная прямыми слешами, не включая сам разделитель. В запросе вида `/path/to/smth` сегментами URI считаются `path`, `to` и `smth`

<a id="node">**Нода**</a> – узел дерева паттернов, сопоставляющий [паттерн](#pattern) с [обработчиком](#action). Может содержать дочерние ноды. [Формат ноды](#nodefmt)
...
<a id="pattern">**Паттерн**</a> – определяет один или несколько [сегментов URI](#seguri), разделенных прямым слешем.
